...
The gem supports three login strategies, selected per host:
...
- **`:proxy`** — the host renders its own e-mail + password form and validates the
  credentials server-to-server against the IdP (`POST /sso/password`), signing the user
  in without ever redirecting the browser to the IdP. Use this when the host is publicly
  reachable but the IdP is network-restricted (e.g. VPN-only), so a browser redirect to
  the IdP would fail.
...
| `SicoobSso::IdentityProvider` | module function | `authorize_url(state:)`, `exchange_code(code)` (redirect); `create_auth_request(email:)`, `poll_auth_request(request_id:)` (push-approval); `authenticate_password(email:, password:)` (proxy) |
...
SicoobSso.configure do |c|
  ...
  c.auth_strategy  = :push_approval                  # :redirect (default) | :push_approval | :proxy
...
| `SSO_AUTH_STRATEGY` | `redirect` (default), `push_approval` or `proxy` | Per host |
...
**All strategies need registration.** `:push_approval` and `:proxy` have no browser
redirect, but the host still authenticates to the IdP with `client_id` + `client_secret`
on the back channel, so the client row (and its secret) is required either way; pass this
app's callback as the `redirect_uri`.
